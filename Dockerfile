...
RUN mkdir /usr/local/schedulytics-frontend
COPY . /usr/local/schedulytics-frontend
WORKDIR /usr/local/schedulytics-frontend
RUN /usr/local/flutter/bin/flutter pub get
RUN /usr/local/flutter/bin/flutter build web

# Stage 2 - Create the run-time image
FROM nginx:stable-alpine
COPY --from=build-env /usr/local/schedulytics-frontend/build/web /usr/share/nginx/html